...
SELECT COUNT(DISTINCT customerNumber) AS totalCustomers FROM customers_cluster_view; -- There are 122 customers

-- How many customers have no purchase history
SELECT * FROM customers_cluster_view WHERE orderNumber IS NULL;
...
SELECT customerNumber, customerName, totalPayments
...
SELECT customerNumber, customerName, COUNT(orderNumber) AS orderCount
FROM customers_cluster_view
...
customers_04 AS (
    ...
    AND customerNumber NOT IN (SELECT customerNumber FROM customers_cluster_view WHERE YEAR(orderDate) = 2003)
),
customers_05 AS (
    SELECT COUNT(DISTINCT customerNumber) AS newCustomers
    FROM customers_cluster_view
...
retained_04 AS (
    ...
    FROM customers_cluster_view
    WHERE YEAR(orderDate) = 2004 
    AND customerNumber IN (SELECT customerNumber FROM customers_cluster_view WHERE YEAR(orderDate) = 2003)
),
retained_05 AS (
    SELECT COUNT(DISTINCT customerNumber) AS retainedCustomers
    FROM customers_cluster_view
...
SELECT SUM(DISTINCT totalPayments) FROM customers_cluster_view; -- Mint Classics made $8,853,839.23 in total sales over the recorded period

-- Sales Rep Performance: Total Payments by Sales Rep
SELECT salesRepEmployeeNumber, lastName, firstName, SUM(DISTINCT totalPayments) AS totalPayments
FROM customers_cluster_view
...
SELECT AVG(DATEDIFF(shippedDate, orderDate)) AS avgDaysToShip
FROM customers_cluster_view
